...
const KatexSelector = ({ equation, setEquation, t }) => {
    const POPULAR_INPUTS = [
        ...
        { text: t('equationModal.doubleIntegral'), equation: '\\iint_{r} f(x,y)' },
        { text: t('equationModal.definiteIntegral'), equation: '\\int_{a}^b f(x)dx' },
        { text: t('equationModal.summation'), equation: '\\sum_{k=1}^n \\alpha_k' },
    ...
    const CALCULUS_SUMS = [
        ...
        { text: t('equationModal.integral'), equation: '\\int f(x)' },
        { text: t('equationModal.doubleIntegral'), equation: '\\iint_{r} f(x,y)' },
        { text: t('equationModal.tripleIntegral'), equation: '\\iiint_{r} f(x,y,z)' },
        { text: t('equationModal.definiteIntegral'), equation: '\\int_{a}^b f(x)dx' },
        { text: t('equationModal.summation'), equation: '\\sum_{k=1}^n \\alpha_k' },
        { text: t('equationModal.limit'), equation: '\\lim_{x \\to a} f(x)' },
        { text: t('equationModal.2dLimit'), equation: '\\lim_{(x,y) \\to (a,b)} f(x)' },
        { text: t('equationModal.3dLimit'), equation: '\\lim_{(x,y,z) \\to (a,b,c)} f(x)' },
        {
        ...
        children: i.inputs.map((i) => (
            <>
                <Tooltip title={i.text}>
                    ...
                    >
                        <KatexRenderer equation={i.equation} inline={true} onClick={() => {}} />
                    </button>
                </Tooltip>
            </>
        )),
