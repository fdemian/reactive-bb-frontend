...
const InlineImageModal = (props) => {
        ...
        if (uploadRet.data) {
            ...
            if (src === null) {
                notification.error({
                    ...
                    description: "Couldn't upload file to the server.",
                    placement: 'topRight',
                });
                setImageURL(null);
                return;
            } else {
                setImageURL(`/static/uploads/${src}`);
            }
        }
        return;
    };
    ...
    // TODO const-ize fallback image.
    return (
        <div>
            <h2>{t("imageModal.upload")}</h2>
            {imageURL ? (
                <>
                    <Image
                        aria-label={t("imageModal.imagePreview")}
                        src={imageURL}
                        alt={t("imageModal.imagePreview")}
                        style={{ width: SIZE_P, height: SIZE_P }}
                ...
                <Dragger {...uploadProps} data-testid="upload-image">
                    <FontAwesomeIcon size="2x" icon={faUpload} />
                    {uploading ? (
                        <Spin />
                    ) : (
                        <>
                            <p className="ant-upload-text">{t("imageModal.clickOrDragToUpload")}</p>
                            <p className="ant-upload-hint">
                                {t("imageModal.maxFilesize")}: <strong>2MB</strong>
                            </p>
                            <p className="ant-upload-hint">
                                {t("imageModal.supportedFileTypes")}: <strong>JPEG,PNG,JPEG2000</strong>
                            </p>
                        </>
                    )}
                </Dragger>
            ...
            <br />
            <div>
                &nbsp; <Checkbox checked={showCaption} onChange={onCheckedChange} /> &nbsp; {t("imageModal.showCaption")}
            </div>
        </div>
    );
