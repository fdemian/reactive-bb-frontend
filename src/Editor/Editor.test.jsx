...
test('<Editor /> > Toolbar and buttons render correctly', async () => {
    ...
    await user.click(screen.getByRole('button', { name: i18t('toolbar.normal')}));
...
test('Code toolbar', async () => {
  ...
  expect(await screen.findByTestId("calliope-editor" )).toBeInTheDocument();
  expect(await screen.findByRole("button", {name: i18t('toolbar.undo')})).toBeInTheDocument();
...
test('<Editor /> > Table toolbar', async () => {
    ...
    expect(screen.getByRole('spinbutton', { name: 'ROWS INPUT' })).toBeInTheDocument();
    expect(screen.getByRole('spinbutton', { name: 'COLS INPUT' })).toBeInTheDocument();
